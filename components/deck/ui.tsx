...
export function Kbd({ children }: { children: React.ReactNode }) {
  return (
    <kbd className="rounded-md bg-latte px-1.5 py-0.5 font-display text-[0.76rem] font-bold">
      {children}
    </kbd>
  );
}
...
export function KbdHints() {
  return (
    <p className="mt-4 hidden text-center text-[0.78rem] text-cocoa sm:block">
...
export function SaveHint() {
  return (
    <div className="mb-4 flex items-center justify-center gap-1.5 text-[0.78rem] text-cocoa">
...
export function StatBar({
  ...
  return (
    <div className="mb-4 flex flex-wrap justify-center gap-[10px]">
      <button
        onClick={onOpenNav}
        className="inline-flex items-center gap-2.5 rounded-[13px] border border-line bg-surface px-4 py-2.5 font-semibold transition hover:border-caramel"
        aria-label={`Open the question navigator — question ${index} of ${total}`}
      >
        <span className="text-[0.9rem] text-cocoa">Question</span>
        <span className="font-display font-extrabold text-coffee">{index}</span>
        <span className="text-cocoa">/</span>
        <span className="font-bold">{total}</span>
      </button>
      <span
        className="inline-flex items-center gap-2 rounded-[13px] border border-green-line bg-green-bg px-4 py-2.5 font-display font-extrabold text-green"
        aria-label={`${correct} marked right`}
      >
        <Check className="h-4 w-4" aria-hidden /> {correct}
      </span>
      <span
        className="inline-flex items-center gap-2 rounded-[13px] border border-red-line bg-red-bg px-4 py-2.5 font-display font-extrabold text-red"
...
export function ProgressBar({ correct, wrong, total }: { correct: number; wrong: number; total: number }) {
  ...
  return (
    <div
      className="mb-[18px] flex h-[8px] overflow-hidden rounded-full bg-latte"
      role="progressbar"
...
export function sheetShell(tone: Tone) {
  return `animate-fade overflow-hidden rounded-card border-2 shadow-warm transition-colors duration-300 ${
    tone === "correct"
      ? "border-green-line bg-[#f5f8ef]"
      : tone === "wrong"
      ? "border-red-line bg-[#fbf1ec]"
  ...
  return (
    <div
      className={`flex items-center justify-between border-b border-line px-[18px] py-3.5 transition-colors duration-300 ${
        tone === "correct" ? "bg-green-bg" : tone === "wrong" ? "bg-red-bg" : "bg-[#f9f2e6]"
      }`}
    >
      <div className="flex items-center gap-1.5">
        ...
        <span
          className={`font-display text-[0.82rem] font-extrabold uppercase tracking-[0.09em] ${
            tone === "correct" ? "text-green" : tone === "wrong" ? "text-red" : "text-coffee"
          }`}
        >
...
export function Stamp({ children, rtl = false }: { children: React.ReactNode; rtl?: boolean }) {
  return (
    <span
      ...
      className={`${rtl ? "thaana" : "font-display"} rounded-[8px] border border-dashed border-caramel/70 bg-surface px-3 py-1.5 text-[0.8rem] font-semibold text-coffee`}
    >
      {children}
    </span>
  );
  ...
  return (
    <>
      ...
      <div
        ...
        aria-modal="true"
        aria-label={title}
        className={`fixed inset-x-0 bottom-0 z-50 flex max-h-[80vh] flex-col rounded-t-sheet bg-surface shadow-[0_-20px_50px_-20px_rgba(0,0,0,.35)] transition-transform duration-300 ${shown ? "translate-y-0" : "translate-y-full"}`}
        style={{ transitionTimingFunction: "cubic-bezier(.22,1,.36,1)" }}
      >
        <div className="border-b border-line px-5 pb-3 pt-4">
          <div className="mx-auto mb-3 h-[5px] w-[46px] rounded-full bg-line" aria-hidden />
          <div className="flex items-center justify-between">
            <h3 className="font-display text-[1.2rem] font-bold">{title}</h3>
            <button
              ref={closeRef}
              onClick={onClose}
              className="grid h-9 w-9 place-items-center rounded-lg text-cocoa hover:bg-black/5 hover:text-coffee"
              aria-label="Close navigator"
            >
              <X className="h-5 w-5" aria-hidden />
            </button>
          </div>
          {subtitle && <div className="mt-1 text-[0.84rem] text-cocoa">{subtitle}</div>}
          {headerExtra}
        </div>
...
export function NavLegend() {
  return (
    <div className="mt-[18px] flex flex-wrap gap-4 border-t border-line pt-4 text-[0.8rem] font-semibold text-cocoa">
      <Legend swatch="bg-teal border-teal" label="Current" />
      <Legend swatch="bg-green-bg border-green-line" label="Right" />
...
export function CompleteCard({
  ...
  return (
    <div className="relative animate-rise overflow-hidden rounded-card border border-line bg-surface p-10 text-center shadow-warm">
      <div className="flex justify-center">
        <KoelMark size={64} className="text-ink" title="The Koveline koel" />
      </div>
      <h3 className="mb-1 mt-4 font-display text-[1.8rem] font-extrabold">You made it through.</h3>
      <p className="mb-5 text-[0.95rem] text-cocoa">
        {wrong > 0
      ...
      <div className="mb-7 flex flex-wrap justify-center gap-[34px]">
        <DoneStat value={correct} label="Right" color="text-green" />
        <DoneStat value={wrong} label="Wrong" color="text-red" />
...
function DoneStat({ value, label, color }: { value: string | number; label: string; color: string }) {
  return (
    <div>
      ...
      <span className="text-[0.78rem] font-bold uppercase tracking-wide text-cocoa">{label}</span>
    </div>
  );
}
...
export function RevealButton({ onClick }: { onClick: () => void }) {
  return (
    <div className="mt-[22px] flex justify-center">
      <button
        onClick={onClick}
        className="inline-flex items-center gap-2 rounded-[14px] bg-teal px-7 py-3.5 font-extrabold text-white shadow-warm-sm transition hover:bg-teal-deep active:translate-y-px"
      >
        Reveal answer
      </button>
    </div>
  ...
  return (
    <div className="mt-5 flex flex-wrap justify-center gap-[11px]">
      <button
        ...
        className={`inline-flex items-center gap-2 rounded-[13px] px-[22px] py-3 font-extrabold transition ${
          status === "wrong" ? "bg-red text-white" : "border-[1.5px] border-red-line bg-red-bg text-red"
        }`}
      >
      ...
      <button
        ...
        className={`inline-flex items-center gap-2 rounded-[13px] px-[22px] py-3 font-extrabold transition ${
          status === "correct" ? "bg-green text-white" : "border-[1.5px] border-green-line bg-green-bg text-green"
        }`}
      >
...
export function NextButton({ isLast, onClick }: { isLast: boolean; onClick: () => void }) {
  return (
    <div className="mt-5 flex justify-center">
      <button
        onClick={onClick}
        className="inline-flex items-center gap-2 rounded-[14px] bg-coffee px-7 py-3.5 font-extrabold text-white transition hover:brightness-110"
      >
